...
class Manager():
    ...
    def perform_login(self) -> None:
        ...
        self.driver.find_element_by_id("username").send_keys(self.login.username)
        self.driver.find_element_by_id ("password").send_keys(self.login.password)
    ...
    def perform_release_delete(self, release:Release) -> None:
        ...
        dialog_div = self.driver.find_element_by_xpath(".//div[@id='delete_version-modal']")  # note NO "#"
        dialog_form = dialog_div.find_element_by_xpath(".//form[@class='modal__form']")
        dialog_input = dialog_form.find_element_by_xpath(".//input[@name='confirm_delete_version']")
        dialog_button = dialog_form.find_element_by_xpath(".//button[@type='submit']")
    ...
    def extract_release_list(self) -> List[Release]:
        ...
        release_list = []
        release_table_list = self.driver.find_elements_by_class_name("table--releases")
        for release_table in release_table_list:
            release_entry_list = release_table.find_elements_by_xpath(".//tbody/tr")
            for release_entry in release_entry_list:
                version_data = release_entry.find_element_by_xpath(".//th/a[contains(@href,'release')]")
                version_text = version_data.text
                datetime_data = release_entry.find_element_by_xpath(".//td/time")
